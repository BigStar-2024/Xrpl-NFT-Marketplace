...
function displayNFTInfo(nft_info_obj){
  ...
  function extractUrisFromNFToken(NFToken_obj){
    ...
    if(!isUndefinedOrNull(nftoken_uri_hex) && nftoken_uri_hex.length > 0){ //if the URI field is present
      ...
      //console.log(extracted_uris_obj);
      uris_obj = {
        ...uris_obj,
        ...extracted_uris_obj
      };
...
export const getNFTInfo = async function(issuer, tokenID) {
    var obj_to_return = {
        type: "XLS20d",
        warnings: []
    };

    if (isUndefinedOrNull(xrpl_obj.client) || !xrpl_obj.client.isConnected()){//if we're not connected to a rippled server (for this "XLS20d")
        await connectToAServer(xrpl_obj); // connect to one
    }
    
    if (!xrpl.isValidAddress(issuer)) { //if the owner address is not valid
        throw new Error("The inserted address is invalid");
    }
    
    //get the NFToken from XRPL
    const NFToken_obj = await getNFToken(issuer, tokenID, xrpl_obj);
    ...
    if(is_default_TOML ||
        ...
        let is_verified = verifyAccountDomain(extracted_toml_objects["ACCOUNTS"], issuer_address, "XLS20d");
    ...
    console.log("composed_metadata_links", composed_metadata_links);
    ...
    obj_to_return["actual_nft_owner"] = issuer;
...
export const doParseNFT = async (issuer, tokenID) => {
    var error_mode = "nft_info";
    var links_HTML = "";
    getNFTInfo(issuer, tokenID).then((nft_info) => {
